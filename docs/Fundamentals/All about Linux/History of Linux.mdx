...
`Fundamentals` · `Reading time: ~8 min` · **Next up:** [Linux Shell and Shell Prompt](./Intro%20to%20Shell%20and%20Terminal)

---

## Introduction

We often talk about Windows and macOS. But did you know there's an *imposter* in the room — a penguin? Yes, that's **Linux**. Some of you may have heard of it, some may not — but it's one of the **most important topics to learn in cybersecurity** today.
...
Unlike commercial operating systems like **Microsoft Windows** and **Apple macOS**, Linux gives users complete control over its source code, letting them **modify**, **redistribute**, or **optimize** the system however they want.

That's what makes it flexible, transparent, and the default choice for developers, ethical hackers, and cybersecurity professionals.

> "Talk is cheap. Show me the code." — **Linus Torvalds**, creator of the Linux kernel
...
| | **Linux** | **Windows** | **macOS** |
| --- | :---: | :---: | :---: |
| Source model | Open source | Closed source | Closed source (Unix-based core) |
| Cost | Free | Paid license | Bundled with Apple hardware |
| Customization | Full — kernel to desktop | Limited | Limited |
| Dominant server market | Yes | Niche | Rare |
| Default in cybersecurity tooling (Kali, Parrot) | Yes | No | No |
| Typical desktop user | Developers, sysadmins, power users | General consumers, enterprises | Creative & consumer users |
...
| Piece | Provides |
| --- | --- |
| **Linux kernel** | Core OS functionality — process scheduling, memory management, device drivers, hardware communication |
| **GNU software** | The tools and utilities needed to actually *use* the kernel — shell (`bash`), compiler (`gcc`), core utilities (`ls`, `cp`, `grep`, ...) |
| **GNU + Linux kernel** | = A complete, usable operating system: **GNU/Linux** |
...
* [Top500 — the world's fastest supercomputers](https://top500.org/)

---

<Admonition type="tip" title="What's Next?">

Now that you know **what Linux is and where it came from**, the next step is learning how to actually interact with it — starting with the **[Linux shell and shell prompt](./Intro%20to%20Shell%20and%20Terminal)**.
